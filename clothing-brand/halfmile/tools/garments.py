"""Halfmile Drop 01 "First Shift": artwork and flat garment mockups, as SVG strings.
...
def art_tee_back(ink=BLACK, accent=BRASS):
    """Bone Shift Tee, back: the line the brand started from."""
    return (fit("NOBODY", 96.9, 72, ink) + fit("FILMS THE", 78.3, 140, ink) + fit("HALF MILE.", 74.1, 205, ink)
            + rules(226, accent)
            + f'<text class="m" x="0" y="284" font-size="15" fill="{ink}">FIRST SHIFT / DROP 01</text>'
            + f'<text class="m" x="300" y="284" font-size="15" fill="{ink}" text-anchor="end">HALFMILE</text>')


def art_hoodie_back(ink=BLACK, accent=BRASS):
    """Split Zip Hoodie, back: the tagline, stacked."""
    return (fit("BUILT FOR", 82.0, 62, ink) + fit("THE LONG", 83.9, 134, ink) + fit("MIDDLE.", 96.7, 216, ink)
            + rules(238, accent)
            + f'<text class="m" x="0" y="296" font-size="15" fill="{ink}">CLOCKED IN / STILL HERE</text>')
...
def split_wordmark(ink, cx, y, w):
    """Wordmark sized so its split line sits exactly on a zipper at x = cx."""
    body, ww = wordmark(ink, "none")
    split_centre = 289          # x of the split line's centre inside wordmark()
    s = w / ww
    return f'<g transform="translate({cx - split_centre * s},{y}) scale({s})">{body}</g>'
...
def products():
    return [
        ...
        dict(id="split-zip-hoodie", svg=zip_hoodie(TAN, split_wordmark(BLACK, 220, 150, 150), label="Split Zip Hoodie, front")),
